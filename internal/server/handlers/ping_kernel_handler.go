...
func (h *PingKernelHttpHandler) HandleRequest(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("Error while pinging kernel", zap.String("kernelId", req.KernelId), zap.Error(err))
		c.AbortWithError(http.StatusInternalServerError, err)
		return
	}

	if resp.Success {
		h.logger.Debug("Successfully pinged kernel.", zap.String("kernelId", req.KernelId))
		c.Status(http.StatusOK)
	} else {
		h.logger.Debug("Failed to ping one or more replicas of kernel.", zap.String("kernelId", req.KernelId))
