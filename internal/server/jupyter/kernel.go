...
func (conn *kernelConnectionImpl) serveMessages() {
	for {
		...
		if responseChannel, ok := conn.responseChannels[responseChannelKey]; ok {
			conn.logger.Debug("Found response channel for websocket message.", zap.String("message-id", kernelMessage.GetParentHeader().MessageId), zap.String("message-type", string(kernelMessage.Header.MessageType)), zap.String("channel", kernelMessage.Channel.String()), zap.String("response-channel-key", responseChannelKey), zap.String("kernel-id", conn.kernelId))
			...
			conn.logger.Warn("Could not find response channel associated with message.", zap.String("message-id", kernelMessage.GetParentHeader().MessageId), zap.String("message-type", string(kernelMessage.Header.MessageType)), zap.String("channel", kernelMessage.Channel.String()), zap.String("response-channel-key", responseChannelKey), zap.String("kernel-id", conn.kernelId))
		}
	}
}
