...
type Synthesizer struct {
	Sources        []EventSource
	GenericSources []EventSource // Non-driver EventSources. Added to `Sources` after first Driver-generated event is processed.
	Tick           int64
...
func NewSynthesizer(opts *WorkloadGeneratorConfig, maxUtilizationWrapper *MaxUtilizationWrapper) *Synthesizer {
	...
	synthesizer := &Synthesizer{
		...
		bufferedEvents:        make(chan *Event),
...
func (s *Synthesizer) AddEventSource(evtSource EventSource) EventSource {
	if s.GenericSources == nil {
		s.GenericSources = make([]EventSource, 0, 2)
	}
...
// Add a Driver as an EventSource to the Synthesizer.
func (s *Synthesizer) AddDriverEventSource(create NewDriver, configs ...func(Driver)) Driver {
	if s.Sources == nil {
		s.Sources = make([]EventSource, 0, 2)
	}

	id := len(s.Sources)
	var eventSource EventSource = create(id, configs...)
...
func (s *Synthesizer) SetEventConsumer(c EventConsumer) {
	s.consumer = c
}
...
func (s *Synthesizer) transitionAndSubmitEvent(evt *Event) {
	if podData, ok := evt.Data.(PodData); ok {
		...
		for _, evtName := range triggered {
			if s.executionMode == 1 {
				...
				sessEvt := &Event{Name: evtName, EventSource: evt.EventSource, OriginalEventSource: evt.OriginalEventSource, Data: eventData, Timestamp: sess.Timestamp, Id: uuid.New().String()}
