...
import {
  ...
  HourglassStartIcon,
  OutlinedCalendarAltIcon, PausedIcon, QuestionIcon,
  SpinnerIcon,
  TimesCircleIcon
...
export const WorkloadDescriptiveIcons: React.FunctionComponent<IWorkloadDescriptiveIcons> = (
  ...
  const getStatusLabel = () => {
    if (props.workload.workload_state == WORKLOAD_STATE_READY) {
      return (<Label icon={<HourglassStartIcon className={text.infoColor_100} />} color="blue">
        Ready
      </Label>);
    }
    ...
    if (props.workload.workload_state == WORKLOAD_STATE_FINISHED) {
        return (<Label icon={<CheckCircleIcon className={text.successColor_100} />} color="green">
          Complete
        </Label>);
    }

    if (props.workload.workload_state == WORKLOAD_STATE_ERRED) {
      return (<Label icon={<TimesCircleIcon className={text.dangerColor_100} />} color="red">
        Erred
      </Label>);
    }

    if (props.workload.workload_state == WORKLOAD_STATE_TERMINATED) {
      return (<Label icon={<ExclamationTriangleIcon className={text.warningColor_100} />} color="orange">
        Terminated
      </Label>);
    }

    return (<Label icon={<QuestionIcon className={text.warningColor_100} />} color="orange">
      Unknown
    </Label>);
  };
