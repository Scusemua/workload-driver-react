...
import { useKernels, useKernelsNamesOnly } from '@app/Providers';
...
export const KernelLogTabContent: React.FunctionComponent<KernelLogTabContentProps> = (
    ...
    const { kernels } = useKernelsNamesOnly();
    ...
    return (
        <Tabs isFilled isSecondary id="kernel-tabs" activeKey={activeKernelTabKey} onSelect={handleKernelTabClick}>
            {kernels.map(
                (
                    kernel: Pick<
                        DistributedJupyterKernel<JupyterKernelReplica>,
                        'kernelId' | 'numReplicas' | 'replicas'
                    >,
                ) => {
                    return (
                        <Tab
