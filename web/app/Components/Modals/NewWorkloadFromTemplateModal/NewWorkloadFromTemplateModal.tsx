...
export interface NewWorkloadFromTemplateModalProps {
  ...
  onClose: () => void;
  onConfirm: (
    workloadRegistrationRequestJson: string
  ) => void;
  // onConfirm: (
...
export const NewWorkloadFromTemplateModal: React.FunctionComponent<
    NewWorkloadFromTemplateModalProps> = (props
  ) => {
    ...
    const getCancelButton = () => {
      if (jsonModeActive) {
        return (<Button key="cancel-application-of-json-to-workload-from-template-button" isDanger variant="secondary"
                        onClick={onDiscardJsonChangesButtonClicked}>
          Discard Changes
        </Button>)
      } else {
        return (<Button key="cancel-submission-of-workload-from-template-button" isDanger variant="secondary"
    ...
    const getActions = () => {
      if (jsonModeActive) {
        ...
        return [
          getSubmitButton(),
          <Button key={"switch-to-json-button"} id={"switch-to-json-button"} icon={<CodeIcon/>} variant={'primary'}
                  onClick={enableJsonEditorMode}>
            Switch to JSON Editor
          </Button>,
          <Button key={"reset-workload-template-form-button"} id={"reset-workload-template-form-button"}
                  icon={<TrashAltIcon/>} variant={'warning'} onClick={onResetFormButtonClicked}>
            ...
            <Flex direction={{'default': 'row'}} spaceItems={{'default': 'spaceItemsXs'}}>
              <FlexItem>
                <Popover
                  ...
                  bodyContent={
                    <div>
                      You can create and register a new workload using a &quot;template&quot;. This allows for a greater
                      degree of
                      dynamicity in the workload&apos;s execution.
                      <br/>
                      <br/>
                      Specifically, templates enable you to customize various properties of the workload, such as the
                      number of sessions, the resource utilization of these sessions,
                      when the sessions start and stop, and the training events processed by the workload&apos;s
                      sessions.
                    </div>
                  }
                >
                          ...
                          <Popover
                            ...
                            bodyContent={
                              <div>
                                ...
                                Please note that if you do specify a seed, then the value must be between 0 and
                                2,147,483,647.
                              </div>
                            }
                          >
                          ...
                          <Popover
                            ...
                            bodyContent={
                              <div>
                                ...
                                Alternatively, setting this quantity to 2.0 will instruct the Workload Driver to spend
                                120
                                seconds on each tick.
                                Setting the quantity to 0.5 will instruct the Workload Driver to spend 30 seconds on
                                each
                                tick.
                              </div>
                            }
