...
export const WorkloadSessionTable: React.FunctionComponent<WorkloadSessionTableProps> = (props) => {
    ...
    const sessions_table_columns: string[] = ["Index", "ID", "Status", "Trainings Completed", "Max MilliCPUs", "Max Memory (MB)", "Max vGPUs"]
                        ...
                        {filteredSessions?.map((session: Session, idx: number) => {
                            return (
                                <Tr key={props.workload?.events_processed[0]?.id} >
                                    ...
                                    <Td dataLabel={sessions_table_columns[5]}><MemoryIcon /> {session?.resource_request.mem_mb} </Td>
